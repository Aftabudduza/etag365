...
function LoadDropdownlist() {
    ...
    if (Plan == 'Basic') {
        ...
        //content += '<option value="Y">1 Year $59.88</option>';

        var content = '<select id="ddlPeriod" class="ddl"><option value="M">1 Month $4.99</option><option value="Y">1 Year $59.88</option></select>';
        ...
        //content += '<option value="Y">1 Year $119.88</option>';

        var content = '<select id="ddlPeriod" class="ddl"><option value="M">1 Month $9.99</option><option value="Y">1 Year $119.88</option></select>';
...
$(document).on('click', '#chkStorage', function () {
    ...
    var nDiscountRate = parseFloat($("#Discount").text()).toFixed(2);
    var nSubTotal = 0;
    var nTotal = 0;
    ...
    if (nTotal === "undefined" || nTotal === "") {
        ...
        if (nCount >= 0) {
            ...
            if (nSubTotal >= 0 && nDiscountRate >= 0) {
                ...
                nDiscount = parseFloat(nDiscountRate);
                nDiscount = parseFloat(nDiscount).toFixed(2);
...
$(document).on("keyup", "#groupcode", function (parameters) {
    ...
    var code = $(this).val().toString().trim();
    var plan = $("input[name=plan]:checked").val();
    var billtype = $("#ddlPeriod").val() == 'Y' ? 'Yearly' : 'Monthly';

    var URL = currentPagePath + "RewardByGroupCodeType";
    ...
    if (code != null && code != "" && code != " ") {
        ...
        $.ajax({
            type: "POST",
            url: URL,
            data: "{ 'code':'" + code + "' , 'plan':'" + plan + "', 'billtype':'" + billtype + "'}",
            ...
            error:
                function (XMLHttpRequest, textStatus, errorThrown) {
                    $("#Discount").text("0.00");
                    $("#percentRatio").text("");
                    var nTotal = parseFloat($("#subTotalCharge").text()).toFixed(2);
                    ...
                    if (nTotal >= 0) {
                        ...
                        $("#TotalAmountCharge").text("0.00");
                        $("#TotalAmountChargeRe").text("0.00");
                    }
...
$(document).on('click', "input[type=radio][name=plan]", function (parameters) {
    ...
    if (Plan == 'Basic') {
        ...
        $("#PlanCharge").text("59.88");
        $("#storageCharge").text("0.00");
        $("#subTotalCharge").text("59.88");
        $("#Discount").text("0.00");
        $("#TotalAmountCharge").text("59.88");
        $("#TotalAmountChargeRe").val("59.88");
        $("#percentRatio").text("");
        $("#groupcode").val("");
        ...
        $("#TotalMonthlyCharge").text("0.00");
        $("#PlanCharge").text("119.88");
        $("#storageCharge").text("0.00");
        $("#subTotalCharge").text("119.88");
        $("#Discount").text("0.00");
        $("#TotalAmountCharge").text("119.88");
        $("#TotalAmountChargeRe").val("119.88");
        $("#percentRatio").text("");
        $("#groupcode").val("");
...
$(document).on('click', '#btnAdd', function () {
    ...
    if (validate == true) {
        ...
        var Obj = getMyDataObj(creditType);
        if (Obj != null) {
            $.ajax({
                type: "POST",
                url: URL,
...
function getMyDataObj(creditType) {
    var nId = 0;
    ...
    if (creditType == 'Credit') {
        ...
        obj = {
            ...
            "State": $("#ddlstateapp").val() == '-1' ? '' : $("#ddlstateapp").val(),
...
function BindBillingInformation(result) {
    if (result == null || result == "") {
        ...
        var Plan = result.SubscriptionType;
...
function GetSavedObjBilling(creditType) {
...
function SaveTransactionTemp(creditType) {
   
    var obj = GetSavedObjBilling(creditType);
...
$(document).on('click', '#btnSubmit2', function () {
    ...
    if (bShow === "undefined" || bShow === "") {
        ...
        if (bShow.toString().toLowerCase() === "false") {
            IsPaid = true; 
        }
        else {
            IsPaid = false; 
    ...
    if (IsPaid == true) {
        notify("danger", "Already paid Billing Payment !!");
    }
    else {
        if ((creditType == 'Credit' || creditType == 'Check') && IsPayable == true) {
            SaveTransactionTemp(creditType);
        }
        else {
            notify("danger", "Total payable amount should be greater than zero !!");
        }
    }    
...
$(document).on('click', '#btnSubmit3', function () {
    ...
    if (IsPaid == true) {
        ...
        if (creditType == 'Credit' || creditType == 'Check') {
            SaveTransactionDirect(creditType);
        }
        else {
            notify("danger", "Please fill out all necessary fields !!");
        }
    }    

});

function SaveTransactionDirect(creditType) {

    var obj = GetSavedObjBilling(creditType);
    var pagePath = currentPagePath + "SaveDirect";
    $.ajax({
        type: "POST",
        url: pagePath,
        data: JSON.stringify({ "obj": obj }),
